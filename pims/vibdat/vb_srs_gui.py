...
class vb_SRS:
    def __init__(self,parent): 
        ...
        self.master.title("vb_srs_gui.py ver 1.6  by Tom Irvine")         
...
#            vs='cm/sec'
        ...
        if(ddmin<=0.01 and 0.01<ddmax):
            plt.text(b*ymax/(2*np.pi*0.01),ymax,'0.01 ' +str("%s" %ds),fontsize=12,rotation=theta) 
 
        if(ddmin<=0.1 and 0.1<ddmax):
            plt.text(b*ymax/(2*np.pi*0.1),ymax,'0.1 ' +str("%s" %ds),fontsize=12,rotation=theta)
    
        if(ddmin<=1 and 1<ddmax):
            plt.text(b*ymax/(2*np.pi*1),ymax,'1 ' +str("%s" %ds),fontsize=12,rotation=theta)
    
        if(ddmin<=10 and 10<ddmax):
            plt.text(b*ymax/(2*np.pi*10),ymax,'10 ' +str("%s" %ds),fontsize=12,rotation=theta)
    
        if(ddmin<=100 and 100<ddmax):
            plt.text(b*ymax/(2*np.pi*100),ymax,'100 ' +str("%s" %ds),fontsize=12,rotation=theta)  
