#!/usr/bin/python
#
#	loook.py 0.6.5 (2006-07-16) - Search in OpenOffice.org files
#
#	Homepage: http://www.danielnaber.de/loook/
#
#	Copyright (C) 2003, 2006 Daniel Naber, Lutz Haseloff
#
...
class Application:
	...
	def createWidgets(self):
		...
		if len(sys.argv) >= 2:
			self.search_path.insert(END, unicode(sys.argv[1], sys.getfilesystemencoding()))
		elif self.search_path_str:
			self.search_path.insert(END, self.search_path_str)
		else:
			self.search_path.insert(END, unicode(os.getcwd(), sys.getfilesystemencoding()))
		...
		if len(sys.argv) >= 2:
			...
			self.search_query.insert(END, unicode(string.join(sys.argv[2:], ' '), enc))
	...
	def showDoc(self, event):
		...
		if os.name == 'nt':
			...
			if not prg and os.name != 'nt':
				tkMessageBox.showwarning('Error', 'Set viewer first.')
			else:
				...
				print cmd
				try:
					res = os.system(cmd.encode('utf-8'))
				except UnicodeError:
					res = os.system(cmd)
				if res != 0:
	...
	def recursiveSearch(self, directory):
		...
		try:
			...
			dir_content.sort(lambda x, y: cmp(string.lower(x), string.lower(y)))
		except OSError, err:
