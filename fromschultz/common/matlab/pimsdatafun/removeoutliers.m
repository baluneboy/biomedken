function [dout,indOutliers] = removeoutliers(din)

% wrapper routine

% The local function does sorting without keeping track of sort order :(
dout = locRemoveoutliers(din);

% Find set difference
delta = setdiff(din,dout);

% Find indices of outliers
indOutliers = find(ismember(din,delta));
dout = din;
dout(indOutliers) = [];

function dataout = locRemoveoutliers(datain)
