function Out = pimsaxlimdlg(message,OptionFlags,PromptString,AxesHandles,XYZstring,DefLim,ApplyFcn)
%pimsaxlimdlg Axes limits dialog box.
%   FIG = pimsaxlimdlg(DlgName,OptionFlags,PromptString, ...
...
%   pimsaxlimdlg(DlgName,OptionFlags,PromptString,AxesHandles,XYZstring,DefLim)
%
%   Notes:
...
elseif strcmp(message,'ApplyCallback'),
   ...
   for i=1:size(ud,1),
      ...
      if isempty(ax) | ~all(BooVec),
         error_str = str2mat('pimsaxlimdlg: Empty or invalid axes handles.', ...
      ...
      if IsAutoChecked,
         ...
         if any(XYZstring(i,:)=='x'),
            ...
            lim = get(ax(1),'XLim');
            numText=locSetSideTextVis(ax,'on'); % Added to set side text visible on
         end
         if any(XYZstring(i,:)=='y'),
            set(ax,'YLimMode','auto')
            lim = get(ax(1),'YLim');
            setscalenote(ax,1); % 1 in 2nd arg means add scale note warning
         end
         ...
         if any(XYZstring(i,:)=='x'),
            set(ax,'XLim',lim)
            numText=locSetSideTextVis(ax,'off'); % Added to set side text visible off
         end
         if any(XYZstring(i,:)=='y'),
            set(ax,'YLim',lim)
            setscalenote(ax,0); % 0 in 2nd arg means remove scale note warning
         end
         if any(XYZstring(i,:)=='z'),
            set(ax,'ZLim',lim)
         end
...
DlgName = message;
% Error checking
% pimsaxlimdlg(message,OptionFlags,PromptString,AxesHandles,XYZstring,DefLim,ApplyFcn)
if ni<2,
   OptionFlags = [0 0];
...
for i=1:rows,
   ...
   ud(i,2) = uicontrol(fig,'Style','edit','String',EditStr,'BackgroundColor','white', ...
    ...
    'UserData',DefLim(i,:),'Callback','pimsaxlimdlg(''EditCallback'')','HandleVisibility','callback');
...
ApplyFcn = [ApplyFcn 'pimsaxlimdlg(''ApplyCallback'');'];
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function numText=locSetSideTextVis(hAx,strOnOff);
hText=findobj(hAx,'type','text');
casTags=get(hText,'tag');
iTextSide=strmatch('TextSide',casTags);
hTextSideAll=hText(iTextSide);
set(hTextSideAll,'visible',strOnOff);
numText=length(hTextSideAll);