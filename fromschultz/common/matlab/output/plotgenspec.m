function h=plotgenspec(t,f,b,sText,strOutType,sReport,clim);
...
%
%h=plotgenspec(t,f,b,sText,strOutType,sReport,clim);
%
...
% $Id: plotgenspec.m 4160 2009-12-11 19:10:14Z khrovat $
...
for r=numAx:-1:1
   ...
   % If transparency capable, then save blue toner from NaNs
   str=version;
   matlabVersion=str2num(str(1:3));
   if 0 % matlabVersion>6
      alpha(not(isnan(b(:,:,r))));
   ...
   axes(hAxes)
   hColorbarLabel=text(1.04,0.5,[sText.casYStub{r} ' PSD Magnitude [log_{ 10}(g^2/Hz)]'],...
   ...
   if r==1
      ...
      set([h.TextComment h.TextTitle],'color',[0 0 0.7]);
   end
   
end

% Lighten axes box color for Adobe PDF measurement capability
colorBox=0.15*[1 1 1];
set(h.AxesALL,'xcolor',colorBox,'ycolor',colorBox);
