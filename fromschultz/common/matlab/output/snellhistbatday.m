function [mg95,cp95,fs,mgBins,sNBIG,sNumHoursBIG,sText]=snellhistbatday(strSensor,Y,M,D,strSubdir,strTitle,blnSmall,varargin);

%snellhistbatday - analyze padhist files
%
% ex.// Y=[2001 2001];
%       M=[6 6];
%       D=[28 29];
...
%
%[mg95,cp95,fs,mgBins,sNBIG,sNumHoursBIG,sText]=snellhistbatday('121f02',2002,10,28,'padhist','Summary #2, Increment 4, Data Set #1',1,cSleep,cWake);
%[mg95,cp95,fs,mgBins,sNBIG,sNumHoursBIG,sText]=snellhistbatday('121f02',2002,10,28,'padhist','Set 02-01, Increment 4',1,cSleep,cWake);
%
...
%
%[mg95,cp95,fs,mgBins,sNBIG,sNumHoursBIG,sText]=snellhistbatday(strSensor,Y,M,D,strSubdir,strTitle,blnSmall,cSleep,cWake);

%written by: Ken Hrovat on 7/30/00
% $Id: snellhistbatday.m 4160 2009-12-11 19:10:14Z khrovat $
...
if ~exist('fc')
   if strcmp(strSensor,'hirap')
      fc=100;
   elseif strcmp(strSensor(1:5),'121f0')
      fc=fs/2.5;
   elseif ( strcmp(strTitle(1:7),'Snell, ') & strcmp(strSensor,'headc') )
      fc=25;
   else
      fc=input('What is cutoff frequency? ');
   end
end
...
for i=1:length(cLabels)
   ...
   if 0
      ...
      %fprintf('\nDID NOT print to %s',strFilename)
