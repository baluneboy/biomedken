function spectrogrambatx(strWhichAx);

switch lower(strWhichAx)
case 'x'
   page=1;
case 'y'
   page=2;
...
for iPlot=1:length(casFiles)/numCat
   ...
   % Relabel for which axis
   hColorbarLabel=findobj(sHandles.Axes11,'tag','TextColorbarLabel11');
   str=get(hColorbarLabel,'string');
   str=strrep(str,'X-Axis',[upper(strWhichAx) '-Axis']);
   set(hColorbarLabel,'string',str);
