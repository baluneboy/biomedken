...
%ex.//
%specpcsabatdaylocalra('121f03',2002,5,23,'tvis',[1 1]); % last arg bln to do both spec and pcsa
%specpcsabatdaylocalra('121f03',2002,5,23,'tvis',[1 0]); % last arg bln to NOT do PCSA
%specpcsabatdaylocalra('121f03',2002,5,23,'tvis',[0 1]); % last arg bln to NOT do spec
%5 input args:
%strSensorID
%numYear
%numMonth
...
while sdnPlotEnd<=(sdnStop+sdnFudge)
   ...
   if ( blnPCSA & exist('B') )
      ...
      if blnGotPCSAdayFile
         ...
         strHistFile=['/sdds/pims2/users/khrovat/temp/' sprintf('%d_%02d_%02d_%s_pcsa_%s.mat',year(sdnFloor),month(sdnFloor),day(sdnFloor),sHeader.SensorID,strTag)];
         fprintf('\n%s did NOT exist.\n',strHistFile)
         [H,freqBins,PSDBins,NUMPSDS]=hist2dspec(f(iKeepFreq),B,sPlot.FLim,[-14 -3]);
         save(strHistFile,'freqBins','PSDBins','H','NUMPSDS','sText');
         blnGotPCSAdayFile=1;
