function specpcsabatdaylocalra(varargin);

%ex.//
%specpcsabatdaylocalra('121f03',2002,5,23,'roadmaps','500p0',[1 1]); % last arg bln to do both spec and pcsa
%specpcsabatdaylocalra('121f03',2002,5,23,'roadmaps','500p0',[1 0]); % last arg bln to NOT do PCSA
%specpcsabatdaylocalra('121f03',2002,5,23,'roadmaps','500p0',[0 1]); % last arg bln to NOT do spec
%5 input args:
%strSensorID
%numYear
%numMonth
...
elseif nargin==7
   [strSensorID,numYear,numMonth,numDay,strTag,strFs,blnProcess]=deal(varargin{:});
   ...
   case 'khrovat.grc.nasa.gov'
      strPrePath='/mnt/yoda/offline/batch/results/';
   case {'ra','ra-new'}
      strPrePath='/sdds/pims2/offline/batch/results/';
      if ~isdir(strPrePath)
         strLocalOutpath='/u1/data/www/plots/batch/';
      end
   case 'elisabeth'
   ...
   %strPathName=[strPrePath strSensorID u yyyy '_' mm u dd u 'padspec' u strTag u]; % OLD
   strPathName=[strPrePath 'year' yyyy u 'month' mm u dd u strSensorID u 'padspec' u strTag u]; % NEW
   isep=findstr(strPathName,filesep);
   %strYM=strPathName(isep(end-4)+1:isep(end-3)-1); % OLD
   strYM=[strPathName(isep(end-6)+5:isep(end-6)+8) '_' strPathName(isep(end-5)+6:isep(end-5)+7)]; % NEW
   %strDay=strPathName(isep(end-3)+4:isep(end-2)-1); % OLD
   strDay=strPathName(isep(end-4)+4:isep(end-3)-1); % NEW
   strInfoPath=strPathName(1:isep(end-3));
   % OLD (next 5 lines)
   %sDir=dir([strInfoPath '*' strTag '*info.mat']);
   %if length(sDir)~=1
   %   error('num of info mat files must be exactly 1')
   %end
   %strInfoFilename=[strInfoPath sDir.name];
   strInfoFilename=getinfofile(strInfoPath,strSensorID,strTag,strFs); % NEW
elseif nargin==8
   [strSensorID,numYear,numMonth,numDay,strTag,strFs,blnProcess,strLocalDir]=deal(varargin{:});
   yyyy=num2str(numYear);
   mm=sprintf('%02d',numMonth);
   dd=sprintf('day%02d',numDay);
   u=filesep;
	strPrePath=[strLocalDir filesep];
   strPathName=[strPrePath 'year' yyyy u 'month' mm u dd u strSensorID u 'padspec' u strTag u]; % NEW
   isep=findstr(strPathName,filesep);
   strYM=[strPathName(isep(end-6)+5:isep(end-6)+8) '_' strPathName(isep(end-5)+6:isep(end-5)+7)]; % NEW
   strDay=strPathName(isep(end-4)+4:isep(end-3)-1); % NEW
...
fprintf('\nPATH: %s',strPathName)
fprintf('\nINFO: %s',strInfoFilename)

% Load info mat-file or m-file
if strcmp('.m',strInfoFilename(end-1:end))
   [PATH,NAME,EXT,VER]=fileparts(strInfoFilename);
   addpath(PATH);
   eval(NAME);
else
...
%sSearch.PathQualifiers.sdnStart=datenum(str2num(strYM(1:4)),str2num(strYM(6:7)),str2num(strDay),0,0,0); % OLD
...
fprintf('\nPlots to span FROM %s TO %s with dT = %f sec. says INFO file.',datestr(sdnStart,0),datestr(sdnStop,0),dT)
...
while sdnPlotEnd<=(sdnStop+sdnFudge)
   ...
   if isempty(index)
      ...
      if blnSpec
         ...
         if 1
            ...
            strExt='eps';
            %strSubDir=[yyyy '_' mm filesep strExt filesep]; % OLD
            %strSubDir=[dd filesep strExt filesep]; % NEW
            strSubDir=[strExt filesep]; % NEWER
   ...
   if ( blnPCSA & exist('B') )
      ...
      if blnGotPCSAdayFile
         ...
         sText.strTitle=['GMT ' datestr(datenum(numYear,numMonth,numDay,0,0,0),1)]; % to correct occasional errant dates that Dick found
         strHistFile=[strPathPCSA sprintf('%d_%02d_%02d_%s_pcsa_%s.mat',numYear,numMonth,numDay,sHeader.SensorID,strTag)];
         fprintf('\n%s did NOT exist.\n',strHistFile)
         [H,freqBins,PSDBins,NUMPSDS]=hist2dspec(f(iKeepFreq),B,sPlot.FLim,[-14 -3]);
         save(strHistFile,'freqBins','PSDBins','H','NUMPSDS','sText');
         blnGotPCSAdayFile=1;
