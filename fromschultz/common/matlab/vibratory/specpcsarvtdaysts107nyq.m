function specpcsarvtdaysts107nyq(varargin);

%ex.//
%specpcsarvtdaysts107nyq('121f03',2002,5,23,'tvis',[1 1 1],franges); % last arg bln to do [blnSpec blnPCSA blnRVT]
%specpcsarvtdaysts107nyq('121f03',2002,5,23,'tvis',[1 0 0],franges); % just do spec
%specpcsarvtdaysts107nyq('121f03',2002,5,23,'tvis',[0 1 0],franges); % just do pcsa
%specpcsarvtdaysts107nyq('121f03',2002,5,23,'tvis',[1 0 1],franges); % do spec and rvt
%5 input args:
%strSensorID
%numYear
%numMonth
...
elseif nargin==7
   [strSensorID,numYear,numMonth,numDay,strTag,blnProcess,franges]=deal(varargin{:});
...
iKeepFreq=find(f<=sHeader.SampleRate/2);
...
while sdnPlotEnd<=(sdnStop+sdnFudge)
   ...
   if isempty(index)
      ...
      %%%%%%%%%%% FOR STS-107 %%%%%%%%%%% 
      sText.strComment=' ';
      sText.casUR{1}='STS-107';
      %iColon=findstr(sText.casUL{1},':');
      %str=sText.casUL{1};
      %if ~isempty(iColon), sText.casUL{1}=str(1:iColon-1); end
      str=sText.casUL{2};
      sText.casUL{2}=strrep(str,'26.3157894737','26.3');
      %%%%%%%%%%% FOR STS-107 %%%%%%%%%%%
      ...
      if blnSpec
         ...
         set(sHandles.AxesALL,'ylim',[0 sHeader.SampleRate/2]);
         set(sHandles.AxesALL,'xlim',TLim);
         set(sHandles.AxesALL,'xtick',tTicks);
         set(sHandles.AxesALL,'ytick',[0:sHeader.SampleRate/2]);
         casYT=cellstr(get(gca,'ytickl'));
         [casYT{2:2:end}]=deal('');
         set(sHandles.AxesALL,'ytickl',casYT);
         ...
         if blnEPS
            ...
            strImageFilename=strrep(strImageFilename,sprintf('_%03d',iPlot),['nyquist' strSampleRate]);
            ...
            set(sHandles.AxesALL,'ytick',[0:26]);
            set(sHandles.AxesALL,'ytickl',[0:26]);
            strImageFilename=strrep(strImageFilename,'nyquist','');
            print('-depsc','-tiff','-r600',[strPath strImageFilename])
         ...
         if blnRVT
            if isempty(franges)
               franges=[0 sHeader.CutoffFreq];
            end
            hImage=findobj(hFigSpec,'tag','Image11');
            hAx=gca;
            posAx=get(hAx,'pos');
            posFig=get(hFigSpec,'pos');
            hText=findobj(hFigSpec,'type','text');
            hTitle=get(hAx,'title');
            t=get(hImage,'xdata');
            for iFrange=1:nRows(franges)
               frange=franges(iFrange,:);
               strFrange=sprintf('%g < f < %g Hz',frange(1),frange(2));
               ugrms=parseval(f(iKeepFreq),B,sHeader.CutoffFreq,frange)/1e-6;
               hFig=figure;
               set(hFig,'pos',posFig);
               hLineRVT=plot(t,ugrms,'k');
               set(gca,'xlim',get(hAx,'xlim'));
               set(gca,'xtick',get(hAx,'xtick'));
               axOld=axis;
               axis([axOld(1:2) 0 axOld(4)]);
               set(gca,'pos',posAx)
               dateaxis('x',15)
               hx=get(hAx,'xlabel');
               xlabel(get(hx,'str'));
               ylabel('RMS Acceleration ( \mug_{RMS} )')
               hc=copyobj(hText,gca);
               set(hc,'fontn','helvetica');
               ht=title(get(hTitle,'str'));
               set(ht,'fontn','helvetica');
               hComment=findobj(hFig,'tag','TextComment');
               set(hComment,'str',[get(hComment,'str') ', ' strFrange],'fontn','helvetica')
               hTCBL=findobj(hFig,'tag','TextColorbarLabel11');
               delete(hTCBL)
            end
         end
