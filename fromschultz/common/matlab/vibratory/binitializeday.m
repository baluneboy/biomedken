function [B,indBmap,sdnPlotTimes,gridMin,gridMax,sdnPreviousEnd,numAx]=binitializeday(sdnPlotBegin,sdnPlotEnd,gridStep,strFirstFile,iKeepFreq,sdn1);
...
%		  sdn1 - sdn for start time from filename
%
...
% $Id: binitializeday.m 4160 2009-12-11 19:10:14Z khrovat $

% Load first file's b matrix and t vector
load(strFirstFile)
if hasstr('spg3',strFirstFile)
   clear b
   b(:,:,1)=bx;
   b(:,:,2)=by;
   b(:,:,3)=bz;
...
if t(1)<(sdnPlotBegin-gridStep)
   warning('first time in first file for this plot is less than desired begin time minus a time step')
end
...
[sdnSnapped,indBmap,iCol]=batsnaptimeday(t,sdnPlotTimes,gridMin,gridMax,gridStep);
for iAx=1:numAx
   B(:,indBmap,iAx)=b(iKeepFreq,iCol,iAx);
end
