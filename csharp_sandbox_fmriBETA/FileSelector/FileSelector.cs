...
namespace Ironic
{
    ...
    /// cref="FileSelector.Zip.ZipFile.AddSelectedFiles(String,String)"/> method is called.
    /// </para>
    ///
    /// <para>
...
public class DemonstrateFileSelector
{
    ...
    public DemonstrateFileSelector(string[] args)
    {
        ...
        for (int i = 0; i < args.Length; i++)
        {
            ...
            if (this._selectionCriteria != null)
            {
                this.f = new Ironic.FileSelector(this._selectionCriteria);
            }
        }
    }
    ...
    public void Run()
    {
        if (this.f == null)
        {
            this.f = new Ironic.FileSelector("name = *.jpg AND (size > 1000 OR atime > 2009-02-14-00:00:00)");
        }
        Console.WriteLine("\nSelecting files:\n" + this.f.ToString());
        //var files = this.f.SelectFiles(this._directory, this._recurse);
        var files = this.f.SelectFiles(@"c:\temp", this._recurse);
        if (files.Count == 0)
        {
